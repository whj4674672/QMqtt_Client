...
Q_MQTT_EXPORT uint qHash(const QMqttTopicFilter &name,
...
public:
  ...
  inline QMqttTopicFilter &operator=(QMqttTopicFilter &&other) Q_DECL_NOTHROW {
    qSwap(d, other.d);
    return *this;
  }
#endif

  inline void swap(QMqttTopicFilter &other) Q_DECL_NOTHROW {
    qSwap(d, other.d);
  }
  ...
  Q_REQUIRED_RESULT bool match(const QMqttTopicName &name,
                               MatchOptions matchOptions = NoMatchOption) const;

  friend Q_MQTT_EXPORT bool
  operator==(const QMqttTopicFilter &lhs,
             const QMqttTopicFilter &rhs) Q_DECL_NOTHROW;
  friend inline bool operator!=(const QMqttTopicFilter &lhs,
                                const QMqttTopicFilter &rhs) Q_DECL_NOTHROW {
    return !(lhs == rhs);
  }
  friend Q_MQTT_EXPORT bool
  operator<(const QMqttTopicFilter &lhs,
            const QMqttTopicFilter &rhs) Q_DECL_NOTHROW;
  friend Q_MQTT_EXPORT uint qHash(const QMqttTopicFilter &filter,

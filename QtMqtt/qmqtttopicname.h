...
uint qHash(const QMqttTopicName &name, uint seed = 0) Q_DECL_NOTHROW;
...
public:
  ...
  inline QMqttTopicName &operator=(QMqttTopicName &&other) Q_DECL_NOTHROW {
    qSwap(d, other.d);
    return *this;
  }
#endif
  ...
  operator==(const QMqttTopicName &lhs,
             const QMqttTopicName &rhs) Q_DECL_NOTHROW;
  friend inline bool operator!=(const QMqttTopicName &lhs,
                                const QMqttTopicName &rhs) Q_DECL_NOTHROW {
    return !(lhs == rhs);
  }
  friend Q_MQTT_EXPORT bool operator<(const QMqttTopicName &lhs,
                                      const QMqttTopicName &rhs) Q_DECL_NOTHROW;
  friend uint qHash(const QMqttTopicName &name, uint seed) Q_DECL_NOTHROW;
